...
torch::Tensor YoloV5::xyxy2xywh(torch::Tensor x)
{
	torch::Tensor y = x.clone();
	y.select(1, 0) = (x.select(1, 0) + x.select(1, 2)) / 2;
	y.select(1, 1) = (x.select(1, 1) + x.select(1, 3)) / 2;
	y.select(1, 2) = (x.select(1, 2) - x.select(1, 0));
	y.select(1, 3) = (x.select(1, 1) - x.select(1, 3));
	return y;
}
...
std::vector<torch::Tensor> YoloV5::prediction(std::vector<cv::Mat> imgs)
{
	...
	std::vector<torch::Tensor> result = prediction(data);  // returns NMSed result
